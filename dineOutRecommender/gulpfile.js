...
gulp.task('run-integration-tests', async () => {
    await gulp.src(`${config.paths.build}/${config.files.spec.integration}`)
        .pipe($.mocha({}))
        .on('error', function (error) {
            $.util.log(err);
            process.exit(1);
...
gulp.task('build', gulp.series('clean', 'compile','copy-config'));
